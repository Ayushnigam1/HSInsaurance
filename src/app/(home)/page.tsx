...
const getText = async () => {
  const INSURANCE_QUERY = defineQuery(`*[_type == "Insurance"]{ insurance, order, description,iconimage }`)
...
const getLandingData=async () => {
  const LANDING_QUERY = defineQuery(`*[_type=="SiteSettingPage"]{ heading, subheading, bannerimage, backgroundimage}`)
  const landingData: LandingData[] = await client.fetch(LANDING_QUERY,{},{ next: { revalidate:60 } })
...
export default async function Home() {
  ...
  return (
    <LandingPage insurances={insurances ?? []} features={features} landingData={landingData} />
  );
}
